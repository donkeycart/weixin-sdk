...
public class Jobs {
    ...
    public String invite(List<String> users, List<Integer> departments, List<Integer> tags) {
        ...
        if (!isEmpty(users)) {
            maps.put("touser", String.join("|", users));
        }

        if (!isEmpty(departments)) {
            maps.put("toparty", departments.stream().map(i -> i.toString()).collect(Collectors.joining("|")));
        }

        if (!isEmpty(tags)) {
            maps.put("toparty", tags.stream().map(i -> i.toString()).collect(Collectors.joining("|")));
        }
